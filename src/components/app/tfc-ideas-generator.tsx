...
import { generateTFCIdeas } from "@/ai/flows/generate-tfc-ideas";
import type { GenerateTFCIdeasOutput } from "@/ai/types/tfc-ideas-types";
...
export default function TfcIdeasGenerator() {
  const [result, setResult] = useState<GenerateTFCIdeasOutput | null>(null);
  ...
  async function onSubmit(values: FormValues) {
    ...
    setResult(null);
    try {
      const ideas = await generateTFCIdeas(values);
      setResult(ideas);
    } catch (error) {
      console.error(error);
      toast({
  ...
  return (
    <div className="space-y-8">
      ...
      {isLoading && (
        <Card>
            <CardHeader>
                <CardTitle>Vos idées de TFC</CardTitle>
            </CardHeader>
